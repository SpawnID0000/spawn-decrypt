...
def load_dev_wallet():
    raw = base64.urlsafe_b64decode(_DEV_WALLET[::-1] + "=" * ((4 - len(_DEV_WALLET[::-1]) % 4) % 4))
...
def authorize(
    ...
    # 3) Prepare caller wallet (or fallback to dev wallet)
    if caller_wallet:
    ...
    # 4) Obtain wrapped_bytes (file-mode or contract-mode)
    if key_file:
        try:
        ...
        for edge in edges:
            ...
            try:
                ...
                try:
                    ...
                    payload = d.json()
                    #print("    [DEBUG] parsed JSON payload")
